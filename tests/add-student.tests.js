...
describe('Add Students page', function () {
  it('Page title', async function () {
    const res = await fetch('http://localhost:8888/add-student');
  ...
  it('Students HTML form', async function () {
    ...
    const nameFieldFound = body.includes('<input id="name" type="text" name="name"/>');
    assert.ok(nameFieldFound, "Field 'name' is missing");

    const emailFieldFound = body.includes('<input id="email" type="email" name="email"/>');
    assert.ok(emailFieldFound, "Field 'email' is missing");

    const buttonAddFound = body.includes('<button type="submit">Add</button>');
  ...
  it('Add valid student', async function () {
    ...
    const studentsReturned = body.includes(
  ...
  it('Add invalid student', async function () {
    ...
    const errMsg = body.includes('Cannot add student. Name and email fields are required!');
